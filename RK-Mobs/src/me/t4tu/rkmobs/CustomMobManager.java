...
public class CustomMobManager {
	...
	private void registerEntities() {
		...
		Map<Object, Type<?>> dataTypes = (Map<Object, Type<?>>) DataConverterRegistry.a().getSchema(DataFixUtils.makeKey(SharedConstants.getGameVersion().getWorldVersion())).findChoiceType(DataConverterTypes.ENTITY).types();
		dataTypes.put("minecraft:custom_zombie", dataTypes.get("minecraft:zombie"));
		EntityTypes.Builder<Entity> a = EntityTypes.Builder.a(NonWanderingZombie::new, EnumCreatureType.MONSTER);
	...
	public org.bukkit.entity.Entity spawnEntity(int index, Location location) {
		try {
			...
			Entity entity = entityTypes.get(index).createCreature(world, null, null, null, new BlockPosition(location.getX(), location.getY(), location.getZ()), EnumMobSpawn.NATURAL, false, false);
